...
SA_VPla             : 1 : Unitless, SA for volume of placenta (VPla)
SA_QPla             : 1 : Unitless, SA for plasma flow to placenta (QPla)
SA_VFet             : 1 : Unitless, SA for fetus volume/weight (VFet) 
SA_VAm              : 1 : Unitless, SA for amniotic fluid volume (VAm)
SA_VLC_Fet          : 1 : Unitless, SA for fractional liver tissue for fetus (VLC_Fet)
SA_QC_Fet           : 1 : Unitless, SA for plasma flow to the fetus (QC_Fet)
SC_QK_P             : 1 : Unitless, SA for plasma flow to kidney during pregnacy (QK_P)
...
double VPla          = 1e-7;
if (GD <= 6)       {VPla = 1e-7;} 
else if (GD <= 10) {VPla = SA_VPla*((N*(8*(GD - 6)))/(1.0e6));} 
else if (GD > 10)  {VPla = SA_VPla*(((N*(32*(exp(-0.23*(GD-10))))) + ((40*(exp(0.28*(GD-10))-1))))/1.0e6);}
...
double QPla          = SA_QPla*(QPla1*(1-Htc)); 
...
double QC_Fet        = SA_QC_Fet*(QPla/(1 + 20000*exp(-0.55*GD)));     
...
double QL            = QLC*QC;                                      
double QK            = QKC*QC;
double QK_P          = SC_QK_P*((-0.0014)*pow(GD, 2) + 0.0308*GD + 0.449);
...
double BW            = PND > 0? SA_BW*(0.0021*PND + BW0) : BW0;
double QCl           = PND > 0? SA_QCl*(0.0123*pow(PND,3) - 0.4059*pow(PND,2) + 3.9661*PND + QCl0) : QCl0;
double QMC           = PND > 0? SA_QMC*((-0.0001)*pow(PND,2) + 0.0051*PND + QMC0) : QMC0;
double QLC           = PND > 0? SA_QLC*(0.0079*PND + 0.2208) : QLC0;
double QKC           = PND > 0? SA_QKC*((-0.0014)*PND + QKC0) : QKC0;
double VFC           = PND > 16? 0.07: SA_VFC*((-0.0012)*pow(PND,2) + 0.0162*PND + 0.1245);
double VMC           = PND > 0? SA_VMC*((-1e-5)*pow(PND,3) + 0.0004*pow(PND,2) + 0.0027*PND + 0.049) : VMC0;
double VLC           = PND > 0? SA_VLC*(0.0384*pow(PND,(0.096))) : VLC0;
double VKC           = PND > 0? SA_VKC*(0.0685*pow(PND,(0.0514))) : VKC0;
double KMilkC        = PND > 0? SA_KMilkC*((-7e-6)*pow(PND,3) + 0.0003*pow(PND,2) - 0.0032*PND + KMilk0) : KMilk0;
...
double BW_pup_W      = SA_BW_pup*BW_pup*N;
double QC_pup_i      = (QCmax*BW_pup)/(BW50 + BW_pup);    
double QC_pup_W      = QC_pup_i*N;                       
double QLC_pup       = SA_QLC_pup*((4e-6)*pow(PND, 3) - 0.0005*pow(PND, 2) + 0.0175*PND + 0.0571);
double QKC_pup       = SA_QKC_pup*((-4e-5)*pow(PND, 2) + 0.0033*PND + 0.0126);
double Htc_pup       = SA_Htc_pup*((-0.0136)*PND + 0.3814);
double VPlasC_pup    = SA_VPlasC_pup*((-6e-6)*pow(PND, 3) + (1e-4)*pow(PND,2) + 0.0006*PND + 0.0454);
