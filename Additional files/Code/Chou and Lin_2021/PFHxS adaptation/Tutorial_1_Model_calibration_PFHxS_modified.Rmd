...
Lout <- Lmod_R %>% 
       init(APlas_free = Init$APlas_free, APTC = Init$APTC, AFil = Init$AFil, 
        ...
        param (Lpars) %>%
        update(atol = 1E-6, maxsteps = 500000) %>%          # origiinal tolerance
       # update(atol = 1e-9, rtol = 1e-9, maxsteps = 5e6) %>% 
...
return (Loutdf)
    ...
    message("[!] pred.B failed: ", e$message)
    # # Fallback dummy data (replace Ltsamp if needed)
    # dummy_time <- seq(0, 504, by = 24)
    # return(data.frame(Time = dummy_time,
    #                   CPlas = rep(1e6, length(dummy_time)),
    #                   CPlas_pup = rep(1e6, length(dummy_time)),
    #                   CMilk = rep(1e6, length(dummy_time))))
  })
...
Lcost<-function (pars){
    ...
    # maternal plasma
    cost<- modCost  (model = outdf.A, obs = OBS.MP1[c("Time", "CPlas")], x ="Time")
    cost<- modCost  (model = outdf.B, obs = OBS.MP2[c("Time", "CPlas")], x ="Time", cost = cost)
    # fetal plasma
    cost<- modCost  (model = outdf.A, obs = OBS.FP1[c("Time", "CPlas_pup")], x ="Time", cost = cost)
    cost<- modCost  (model = outdf.B, obs = OBS.FP2[c("Time", "CPlas_pup")], x ="Time", cost = cost)
    # milk band (stomach contents)
    cost <- modCost (model = outdf.A, obs = OBS.MB1[c("Time", "CMilk")], x = "Time", cost = cost)
    cost <- modCost (model = outdf.B, obs = OBS.MB2[c("Time", "CMilk")], x = "Time", cost = cost)
...
Ltheta_PFHxS.int <- log(c(
    ...
    PMilkM                         = 1.9,
    PMilkP                         = 0.007, #Haug et al., 2011 (human breast milk vs plasma; PFHxS M:P ≈0.007:1)
    PAMilkC                        = 0.5,
    K0C                            = 1,
    KabsC                          = 2.12,
    KunabsC                        = 7.05e-5,
...
```{r}
Ltheta_PFHxS <- log(c(
    #Vmax_baso_invitro              = 393.45, #PFOS
    #Km_baso                        = 27.2,
    Vmax_apical_invitro            = 4164, #PFHxS
    #Km_apical                      = 278, #PFOS
    #RAFbaso                        = 1.90,
    #RAFapi                         = 4.15, #PFOS
    #KeffluxC                       = 2.09,
    ...
    #KeffluxC_p                     = 2.09,
    #Kdif_pup_0                     = 0.001#,
    #Vmax_baso_invitro_p            = 393.45, #PFOS
    #Km_baso_p                      = 27.2,
    #Vmax_apical_invitro_p          = 4164 #PFHxS
    #Km_apical_p                    = 278
))
```
...
Ltheta <- Ltheta_PFOS
#Ltheta <- Ltheta_PFHxS
#Ltheta <- Ltheta_PFHxS.int

# Clip any out-of-bounds starting values
#Ltheta <- pmin(pmax(Ltheta, log(1e-4)), log(1e2))

# Define prior values (log-scale)
Ltheta_prior <- Ltheta
lower_bounds <- Ltheta_prior - 2
upper_bounds <- Ltheta_prior + 2

LFit <- modFit(f = Lcost, p = Ltheta, method ="Marq",
               # constrain params to biologically-plausible range
               #lower = lower_bounds,
               #upper = upper_bounds,
               control = nls.lm.control(nprint = 1))
```
...
obs_all <- bind_rows(
  ...
  #join SDs
  left_join(Data_0 %>% select(Time, SD, Conc), by = c("Time",  "Conc")) 
...
# 1. Dam serum plot
p_dam <- ggplot() +
  geom_line(data = filter(sim_long, Compartment == "Dam serum (model)"),
            aes(x = Time, y = Conc_pred, color = Compartment), linewidth = 1) +
  geom_point(data = filter(obs_all, Compartment == "Dam serum (obs)"),
             aes(x = Time, y = Conc, color = Compartment, shape = Compartment), size = 4) +
  geom_errorbar(aes(x = Time, ymin = Conc - SD, ymax = Conc + SD, color = Compartment, shape = Compartment), 
                data = filter(obs_all, Compartment == "Dam serum (obs)"), 
                width = 5) +
  scale_x_continuous(limits = c(530, 1000)) +
  scale_y_continuous(limits = c(0, 250)) +
  scale_color_manual(values = custom_colors) +
  facet_wrap(~Dose) +
  labs(title = "Dam Serum", x = "Time (hr)", y = "PFHxS (µg/mL)") +
  theme_bw(base_size = 15)

# 2. Pup serum plot
p_pup <- ggplot() +
  geom_line(data = filter(sim_long, Compartment == "Pup serum (model)"),
            aes(x = Time, y = Conc_pred, color = Compartment), linewidth = 1) +
  geom_point(data = filter(obs_all, Compartment == "Pup serum (obs)"),
             aes(x = Time, y = Conc, color = Compartment, shape = Compartment), size = 4) +
  geom_errorbar(aes(x = Time, ymin = Conc - SD, ymax = Conc + SD, color = Compartment, shape = Compartment), 
                data = filter(obs_all, Compartment == "Pup serum (obs)"), 
                width = 5) +
  scale_x_continuous(limits = c(530, 1000)) +
  scale_y_continuous(limits = c(0, 75)) +
  scale_color_manual(values = custom_colors) +
  facet_wrap(~Dose) +
  labs(title = "Pup Serum", x = "Time (hr)", y = "PFHxS (µg/mL)") +
  theme_bw(base_size = 15)

# 3. Milk band plot
p_milk <- ggplot() +
  geom_line(data = filter(sim_long, Compartment == "Milk band (model)"),
            aes(x = Time, y = Conc_pred, color = Compartment), linewidth = 1) +
  geom_point(data = filter(obs_all, Compartment == "Milk band (obs)"),
             aes(x = Time, y = Conc, color = Compartment, shape = Compartment), size = 4) +
  geom_errorbar(aes(x = Time, ymin = Conc - SD, ymax = Conc + SD, color = Compartment, shape = Compartment), 
                data = filter(obs_all, Compartment == "Milk band (obs)"), 
                width = 5) +
  scale_x_continuous(limits = c(530, 1000)) +
  scale_color_manual(values = custom_colors) +
  facet_wrap(~Dose) +
  labs(title = "Milk Band", x = "Time (hr)", y = "PFHxS (µg/mL)") +
  theme_bw(base_size = 15)

# Combine plots
ggarrange(p_dam, p_pup, p_milk, ncol = 1, common.legend = TRUE, legend = "bottom")
