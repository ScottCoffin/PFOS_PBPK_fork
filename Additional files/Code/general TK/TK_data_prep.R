...
tk_params <- tk_params_init %>% 
  ...
  select(chem, sex, species, strain, route, CLTot_L_kg_day, VDss_L_kg, HLe_invivo, kabs) %>% 
  #select(-c(Study, contains("mismatch"),
   #         contains("pubmed"), contains("author_year"))) %>% 
  mutate(across(c(chem, sex, species, strain, route),  as.factor)) %>% 
  rename_with(str_to_title) %>% 
  rename(PFAS = Chem,
         `Clearance (L/kg-d)` = Cltot_l_kg_day,
         `Volume of Distribution (L/kg)` = Vdss_l_kg,
         `Half-Life (hr)` = Hle_invivo,
         `Absorption Coefficient (unitless)` = Kabs) %>% 
