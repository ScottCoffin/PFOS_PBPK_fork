...
initial_table_data <- data.frame(
  ...
  Exposure_Duration_Days = as.integer(c(30, 10, 5, 3)),
  Time_Serum_Collected_hr = as.integer(c(744, 264, 144, 96)),
 ...
 #####################################################################################################
  ...
  simulation_results <- eventReactive(input$run, {
    ...
    #  bw <- row$Body_Weight_kg
      ...
      exposure_duration <- exposure_duration_days * 24
      pfas <- row$PFAS
      sex <- row$Sex
      ...
      if (model_type == "PBPK") {
        ...
        cat("Exposure Duration:", exposure_duration, "hours\n")
        ...
        conc <- if (!is.null(ka)) {
          map_dbl(times, ~ full_conc(dose, vd, ke, ka, interval, n_doses, .x, exposure_duration))
        } else {
          map_dbl(times, ~ simplified_conc(dose, vd, ke, interval, n_doses, .x, exposure_duration))
        }
